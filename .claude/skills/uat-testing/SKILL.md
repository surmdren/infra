---
name: uat-testing
description: 用户验收测试（UAT）。从真实用户视角出发，基于 PRD 用户故事和用户角色定义，执行端到端浏览器自动化测试（Playwright），验证产品功能符合用户期望，操作流程自然顺畅，关键业务路径无阻断。若 Chrome DevTools MCP 可用，优先用它直接操作真实浏览器（检查交互、截图、验证页面状态），比 Playwright 脚本更直观。输出用户视角的验收报告，存放到 UAT/ 目录。应在 release-qa 技术验收通过后、正式发布前执行。当用户提到"UAT"、"用户验收"、"用户测试"、"用户侧测试"、"uat testing"、"用户场景测试"、"验收用户故事"时触发。
---
