...
description:
  当需要真实浏览器环境时使用此 skill：访问需要登录的网站、操作网页界面、抓取反爬平台内容（小红书、微博、推特等）、读取动态渲染页面、视频采帧分析、需要保持登录态的任何操作。
  简单的搜索（WebSearch）或公开页面抓取（WebFetch）不需要触发此 skill。
  触发关键词：CDP、Chrome 远程调试、需要登录、小红书、微博、动态页面、浏览器自动化、web-access。
metadata:
  author: 一泽Eze
  version: "2.4.0"
---
...
```bash
bash "${CLAUDE_SKILL_DIR:-$HOME/.claude/skills/web-access}/scripts/check-deps.sh"
```
